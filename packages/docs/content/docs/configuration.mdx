...
The administrator studio and learner portal can use different access codes. Keep
`ACCESS_CODE` private for administrators, then optionally set a separate code for
students:

```bash
LEARNER_ACCESS_CODE=your-student-code
```

Students enter through `/learn`. They can only list and open courses that an
administrator has published. Provider settings, course generation, editing, and
exports remain administrator-only. Leave `LEARNER_ACCESS_CODE` empty if published
courses should be publicly accessible.
